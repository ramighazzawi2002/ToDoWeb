...
const sendEmailNotification = async (
  ...
  try {
    ...
    const taskList = tasks
      .map(
        (task, index) =>
          `${index + 1}. ${task.taskTitle} - Due: ${new Date(
            task.dueDate
          ...
          <ul style="list-style-type: none; padding: 0;">
            ${tasks
              ...
              <li style="background: #f5f5f5; margin: 10px 0; padding: 15px; border-radius: 5px;">
                ...
                <span style="color: #666;">تاريخ الاستحقاق: ${new Date(
                  task.dueDate
                ).toLocaleString("ar-EG")}</span><br>
                <span style="color: #666;">قائمة المهام: ${
                  task.todoListTitle
                }</span>
              </li>
            `
...
export const startOptimizedTaskReminderCron = () => {
  ...
  nodeCron.schedule("*/5 * * * *", async function () {
    ...
    try {
      ...
      const dueTasks = await CronCacheService.getDueTasks(
      ...
      for (const [userId, userTasks] of tasksByUser) {
        ...
        try {
          ...
          if (user && user.email) {
            ...
            console.log(
              `📧 Sending email to ${user.email} with subject: ${emailSubject}`
            );
        ...
        for (const taskInfo of userTasks) {
          await CronCacheService.markAsNotified(
    ...
    try {
      ...
      for (const task of overdueTasks) {
        ...
        const wasNotified = await CronCacheService.wasRecentlyNotified(
          taskId,
          "overdue",
          60 // 1 hour cooldown for overdue notifications
        );
      ...
      for (const [userId, userTasks] of overdueTasksByUser) {
        ...
        try {
          console.log(
            `🔍 Attempting to send overdue email for user ${userId}...`
          );
          const user = await UserModel.findById(userId).select(
            "email firstName lastName"
          );
          ...
          if (user && user.email) {
            ...
            console.log(
              `📧 Sending overdue email to ${user.email} with subject: ${emailSubject}`
            );
            await sendEmailNotification(
              user.email,
              userName,
