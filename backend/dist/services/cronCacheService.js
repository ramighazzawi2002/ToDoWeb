...
export class CronCacheService {
     ...
     */
    static async getDueTasks(thirtyMinutesFromNow, now) {
        ...
        try {
            ...
            // Cache the result
            await redis.setex(cacheKey, this.CACHE_TTL.TASKS, JSON.stringify(dueTasks));
            return dueTasks;
        }
        catch (error) {
     ...
     */
    static async getOverdueTasks(now) {
        ...
        try {
            ...
            const overdueTasks = await ToDoItemModel.find({
                ...
                .lean(); // Use lean() for better performance
            // Cache the result
            await redis.setex(cacheKey, this.CACHE_TTL.TASKS, JSON.stringify(overdueTasks));
            return overdueTasks;
        }
        catch (error) {
     ...
     */
    static async wasRecentlyNotified(taskId, notificationType, cooldownMinutes = 25) {
     ...
     */
    static async markAsNotified(taskId, notificationType) {
        const key = `${this.CACHE_KEYS.LAST_NOTIFICATION_PREFIX}${notificationType}:${taskId}`;
        const now = Date.now();
        try {
     ...
     */
    static async getUserTasksSummary(userId) {
     ...
     */
    static async cacheUserTasksSummary(userId, taskIds) {
        const key = `${this.CACHE_KEYS.USER_TASKS_PREFIX}${userId}`;
        try {
     ...
     */
    static async invalidateTasksCache() {
        try {
            const patterns = [
